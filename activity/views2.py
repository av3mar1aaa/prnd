...
def all_user_reports(request):
  if is_admin(request.user):
    ...
    not_filtered_user_list = [u for u in not_filtered_user_list if not u.groups.filter(id=10).exists()]
    not_filtered_user_list.sort(key=lambda u: (
        (u.groups.first().name if u.groups.first() else ""),
        (u.last_name or ""),
        (u.first_name or ""),
        u.username,
      ))
  elif is_zavlab(request.user):
    g = request.user.groups.first()
    if not g:
      return NotAllowed(request)
    not_filtered_user_list = list(g.user_set.all().order_by('last_name'))
  else:
    return NotAllowed(request)

  return render(request, "acts/all_user_reports.html", {
    "user_list": not_filtered_user_list,
  })
  ...
  if not request.method == 'POST':
    ...
    if not form_errors:
      ...
      for tempyear in year_list:
        ...
        detailed_reports += "<br><b>" + str(tempyear) + ": </b>" + (ys.detailed_report or "")
...
def year_reports(request, userid, year, wbreport=False):
  ...
  if is_laboratory(request.user):
    not_filtered_user_list = request.user.groups.all()[0].user_set.all().order_by('last_name')
